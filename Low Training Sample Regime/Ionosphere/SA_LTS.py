...
total_data, y = ionosphere[:,range(0,ionosphere.shape[1]-1)], ionosphere[:,ionosphere.shape[1]-1].astype(str)
...
total_label = y.astype(int)


X_norm = total_data
X_norm[:,range(2,total_data.shape[1])] = (total_data[:,range(2,total_data.shape[1])]-np.min(total_data[:,range(2,total_data.shape[1])],0))/(np.max(total_data[:,range(2,total_data.shape[1])],0)-np.min(total_data[:,range(2,total_data.shape[1])],0))
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = DecisionTreeClassifier(min_samples_leaf = 1, random_state=42, max_depth = 2, ccp_alpha = 0.0)
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = RandomForestClassifier( n_estimators = 10000, max_depth = 4, random_state=42)
        clf.fit(X_train, y_train.ravel())
        Y_PRED = clf.predict(X_test)
        F1SCORE = f1_score(y_test, Y_PRED, average='macro')
        final_f1_score.append(F1SCORE)  
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = SVC(C = 2.6, kernel='rbf', decision_function_shape='ovr', random_state = 42)
        clf.fit(X_train, y_train.ravel())
        Y_PRED = clf.predict(X_test)
        F1SCORE = f1_score(y_test, Y_PRED, average='macro')
        final_f1_score.append(F1SCORE)  
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        INA = 0.680 #np.arange(0.01, 0.99,0.01)
        DT = 0.969 #np.arange(0.01, 0.99, 0.01)
        EPSILON_1 = 0.164
