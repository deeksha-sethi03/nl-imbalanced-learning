...
X, y = wine[:,range(1,wine.shape[1])], wine[:,0].astype(str)
y = np.char.replace(y, '1', '0', count=None)
y = np.char.replace(y, '2', '1', count=None)
y = np.char.replace(y, '3', '2', count=None)
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        INA = 0.79 #np.arange(0.01, 0.99,0.01)
        ...
        clf = DecisionTreeClassifier(min_samples_leaf = 1, random_state=42, max_depth = 6, ccp_alpha = 0.0)
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = SVC(C = 16.0, kernel='rbf', decision_function_shape='ovr', random_state = 42)
        clf.fit(FEATURE_MATRIX_TRAIN, y_train.ravel())
