...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        INA = 0.02 #np.arange(0.01, 0.99,0.01)
        DT = 0.07 #np.arange(0.01, 0.99, 0.01)
        EPSILON_1 = 0.238
        ...
        clf = DecisionTreeClassifier(min_samples_leaf = 2, random_state=42, max_depth = 5, ccp_alpha = 0.0)
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = SVC(C = 2.4, kernel='rbf', decision_function_shape='ovr', random_state = 42)
        clf.fit(FEATURE_MATRIX_TRAIN, y_train.ravel())
