...
Dataset: Free Spoken Digit Datatset (FSDD) - Time Series
Code Description:  
...
y = np.zeros((len(data_length)), dtype='int')
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        INA = 0.34 #np.arange(0.01, 0.99,0.01)
        DT = 0.499 #np.arange(0.01, 0.99, 0.01)
        EPSILON_1 = 0.178
        ...
        clf = DecisionTreeClassifier(min_samples_leaf = 1, random_state=42, max_depth = 10, ccp_alpha = 0.006138392857142856)
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = RandomForestClassifier( n_estimators = 1000, max_depth = 8, random_state=42)
        clf.fit(FEATURE_MATRIX_TRAIN, y_train.ravel())
...
for NUM_SAMPLES in range(1, NUM_SAMPLES_PER_CLASS+1):
    ...
    for num_trials in range(0, NUM_TRIALS):
        ...
        clf = SVC(C = 6.1, kernel='linear', decision_function_shape='ovr', random_state = 42)
        clf.fit(FEATURE_MATRIX_TRAIN, y_train.ravel())
